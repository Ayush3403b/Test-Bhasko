...
import {
  City, SolarPackage, ProductBrand, FAQ, FinancingOption,
  SubsidyInformation, AMCPlan, BlogPost, JobOpening
} from './models';
...
export async function seedDatabase() {
  ...
  const cities = [
    { name: 'Patna', slug: 'patna', state: 'Bihar', solarYieldKwhPerKwPerDay: 4.6, avgTariff: 7.0, discom: 'SBPDCL / NBPDCL', subsidyApplicable: true,
      ...
      metaDescription: 'Calculate your solar savings for Patna. Premium rooftop solar installation in Patna with PM Surya Ghar subsidy.',
  ...
  /* ---------- FAQs ---------- */
  const faqs = [
    { category: 'General', order: 1, question: 'How much does a rooftop solar system cost in 2025?', answer: 'Costs vary by capacity, equipment and package. Typical on-grid residential systems range from approximately ₹52,000–₹72,000 per kW (indicative, pre-subsidy).' },
    { category: 'General', order: 2, question: 'How much subsidy can I get under PM Surya Ghar Yojana?', answer: 'PM Surya Ghar Yojana provides central financial assistance: ₹30,000 per kW up to 2 kW, and capped at ₹78,000 for 3 kW and above for typical residential installations.' },
    { category: 'Technical', order: 3, question: 'How much roof area is required for solar?', answer: 'As a rule of thumb, 1 kW requires about 85–100 sq ft of shade-free roof area. A 3 kW system needs roughly 300 sq ft, and a 5 kW system around 500 sq ft.' },
    { category: 'Financial', order: 4, question: 'What is the typical payback period for rooftop solar in India?', answer: 'Most on-grid residential systems pay back in 3.5 to 5 years through electricity bill savings plus applicable subsidies.' },
    { category: 'Process', order: 5, question: 'How long does installation take?', answer: 'Physical installation typically takes 2–5 days depending on system size. DISCOM approvals and net metering can take 2–6 weeks, handled end-to-end.' },
    { category: 'Support', order: 6, question: 'What happens after installation if something breaks?', answer: 'All Bhasko systems include warranty and AMC options. You can raise service requests anytime.' },
  ];
  for (const f of faqs) await FAQ.findOneAndUpdate({ question: f.question }, f, { upsert: true });

  /* ---------- Financing ---------- */
  const fin = [
    { name: 'Solar Easy EMI', provider: 'Partner Banks & NBFCs', type: 'emi' as const, interestRate: 9.5, tenureMonths: 84, minLoanAmount: 50000, maxLoanAmount: 2000000, downPaymentPct: 20, description: 'EMI starting at ~₹1,500 per kW.' },
    ...
    { name: 'Lease / PPA', provider: 'Bhasko Capital Partners', type: 'ppa' as const, description: 'Pay per unit generated at a rate lower than your grid tariff. Zero upfront investment.' },
  ];
